...
const dataPath = path.join(sysRoot, ".otakocraft-launcher");
...
exports.load = function () {
  ...
  if (doLoad) {
    ...
    try {
      ...
      logger.error(err);
      logger.info(
        "El archivo de configuración contiene JSON con formato incorrecto o está dañado."
      );
 ...
 */
exports.addCrackedAccount = function (username) {
  ...
  config.selectedAccount = randomUUID;
  config.authenticationDatabase[randomUUID] = {
    type: "cracked",
    displayName: username.trim(),
    uuid: randomUUID,
  };
  return config.authenticationDatabase[randomUUID];
};
