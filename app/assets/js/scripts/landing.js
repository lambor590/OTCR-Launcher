...
document
  ...
  .addEventListener("click", async (e) => {
    loggerLanding.info("Iniciando juego...");
    try {
      ...
      loggerLanding.error(
        "Error no controlado durante el proceso de inicio.",
        err
      );
      showLaunchFailure(
        "Error durante el inicio",
        "Ver consola (CTRL + Shift + i) para más detalles."
      );
    }
  });
...
function updateSelectedServer(serv) {
  ...
  server_selection_button.innerHTML =
    "\u2022 " +
    (serv != null ? serv.rawServer.name : "Ningún servidor seleccionado");
  if (getCurrentView() === VIEWS.settings) {
...
const checkMaintenance = async function () {
  const mntInfo = await DistroAPI.getDistribution();
  return [
...
const refreshMojangStatuses = async function () {
  loggerLanding.info("Refrescando los estados de Mojang...");
  ...
  if (response.responseStatus === RestResponseStatus.SUCCESS) {
    ...
    loggerLanding.warn(
      "No se puede actualizar el estado de los servicios de Mojang."
    );
...
const refreshServerStatus = async (fade = false) => {
  loggerLanding.info("Actualizando estado del servidor");
  ...
  try {
    ...
    loggerLanding.warn(
      "No se puede actualizar el estado del servidor, asumiendo que está fuera de línea."
    );
...
function showLaunchFailure(title, desc) {
  setOverlayContent(title, desc, "Vale");
  setOverlayHandler(null);
  toggleOverlay(true);
...
async function asyncSystemScan(effectiveJavaOptions, launchAfter = true) {
  setLaunchDetails("Comprobando la información del sistema...");
  toggleLaunchArea(true);
  setLaunchPercentage(0, 100);
  ...
  if (jvmDetails == null) {
    ...
    setOverlayContent(
      "No se encontró ninguna instalación<br>de Java compatible",
      `Para entrar al servidor, necesitar tener instalada una versión de 64 bits de Java ${effectiveJavaOptions.suggestedMajor}. ¿Quieres que lo instalemos por ti?`,
      "Instalar Java",
    ...
    setOverlayHandler(() => {
      ...
      try {
        ...
        loggerLanding.error("Error no controlado en la descarga de Java", err);
        showLaunchFailure(
          "Error durante la descarga de Java",
          "Ver consola (CTRL + Shift + i) para más detalles."
        );
      }
    });
    setDismissHandler(() => {
      $("#overlayContent").fadeOut(250, () => {
        ...
        setOverlayContent(
          "Se requiere Java<br>para iniciar",
          `Una instalación de 64 bits válida de Java ${effectiveJavaOptions.suggestedMajor} es necesaria. Puedes pedir ayuda a The Ghost#6088 en Discord o buscar en internet.`,
          "Entiendo",
          "Volver"
        );
        setOverlayHandler(() => {
...
async function downloadJava(effectiveJavaOptions, launchAfter = true) {
  ...
  if (asset == null) {
    throw new Error("No se pudo encontrar la distribución de OpenJDK.");
  }
  ...
  if (received != asset.size) {
    loggerLanding.warn(
      `Descarga de Java: Se esperaban ${asset.size} bytes pero se recibieron ${received}`
    );
    if (!(await validateLocalFile(asset.path, asset.algo, asset.hash))) {
      log.error(`Los hashes no coinciden, ${asset.id} podría estar dañado.`);
      // Don't know how this could happen, but report it.
      throw new Error(
        "El JDK descargado tiene un hash incorrecto, es posible que el archivo esté dañado."
      );
    }
  }
...
async function dlAsync(login = true) {
  ...
  try {
    ...
    loggerLaunchSuite.error(
      "No se puede actualizar el índice de distribución.",
      err
    );
    showLaunchFailure(
      "Error Fatal",
      "No se pudo cargar una copia del índice de distribución. Consulte la consola (CTRL + Shift + i) para obtener más detalles."
    );
    return;
  }
  ...
  if (login) {
    if (ConfigManager.getSelectedAccount() == null) {
      loggerLanding.error("Debe iniciar sesión en una cuenta.");
      return;
    }
  }

  setLaunchDetails("Espera por favor...");
  toggleLaunchArea(true);
  setLaunchPercentage(0, 100);
  ...
  fullRepairModule.childProcess.on("error", (err) => {
    ...
    showLaunchFailure(
      "Error durante el inicio",
      err.message || "Ver consola (CTRL + Shift + i) para más detalles."
    );
  });
  fullRepairModule.childProcess.on("close", (code, _signal) => {
    if (code !== 0) {
      loggerLaunchSuite.error(
        `Se cerró el módulo de reparación completa con el código ${code}, asumiendo un error.`
      );
      showLaunchFailure(
        "Error durante el inicio",
        "Ver consola (CTRL + Shift + i) para más detalles."
  ...
  loggerLaunchSuite.info("Validando archivos.");
  setLaunchDetails("Validando archivos...");
  ...
  try {
    ...
    loggerLaunchSuite.error("Error durante la validación del archivo.");
    showLaunchFailure(
      "Error durante la verificación de archivos",
      err.displayable || "Ver consola (CTRL + Shift + i) para más detalles."
    );
    return;
  }
  ...
  if (invalidFileCount > 0) {
    ...
    try {
      ...
      loggerLaunchSuite.error("Error durante la descarga del archivo.");
      showLaunchFailure(
        "Error durante la descarga del archivo.",
        err.displayable || "Ver consola (CTRL + Shift + i) para más detalles."
      );
      return;
    }
    ...
    loggerLaunchSuite.info(
      "No hay archivos no válidos, omitiendo la descarga."
    );
  }
  ...
  setLaunchDetails("Preparando inicialización...");
  ...
  if (login) {
    ...
    loggerLaunchSuite.info(
      `Enviando cuenta seleccionada (${authUser.displayName}) al ProcessBuilder.`
    );
    ...
    const gameErrorListener = function (data) {
      ...
      if (
        ...
        loggerLaunchSuite.error(
          "El lanzamiento del juego falló, LaunchWrapper no se descargó correctamente."
        );
        showLaunchFailure(
          "Error durante el inicio",
          "El archivo principal, LaunchWrapper, no se pudo descargar correctamente. Como resultado, el juego no puede iniciarse.<br><br>Para solucionar este problema, apague temporalmente su software antivirus y vuelva a iniciar el juego."
        );
      }
    };
    ...
    try {
      ...
      showLaunchFailure(
        "Error durante el inicio",
        "Consulte la consola (CTRL + Shift + i) para obtener más detalles."
      );
    }
  }
}
...
function setNewsLoading(val) {
  if (val) {
    const nLStr = "Comprobando noticias";
    let dotStr = "..";
...
function displayArticle(articleObject, index) {
  ...
  newsArticleAuthor.innerHTML = "de " + articleObject.author;
...
async function loadNews() {
  ...
  if (!distroData.rawDistribution.rss) {
    loggerLanding.debug("No se proporcionó una fuente RSS.");
    return null;
  }
