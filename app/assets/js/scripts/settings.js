...
function disableAddCrackedAccount() {
  document.getElementById("settingsAddCrackedAccount").disabled = true;
}
...
ipcRenderer.on(MSFT_OPCODE.REPLY_LOGIN, (_, ...arguments_) => {
  if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
    ...
    switchView(getCurrentView(), viewOnClose, 500, 500, () => {
      ...
      setOverlayContent(
        "Algo salió mal",
        "La autenticación de Microsoft falló. Inténtalo de nuevo.",
        "OK"
      );
      setOverlayHandler(() => {
        toggleOverlay(false);
    ...
    if (Object.prototype.hasOwnProperty.call(queryMap, "error")) {
      switchView(getCurrentView(), viewOnClose, 500, 500, () => {
        ...
        console.log(
          "Error al obtener el authCode, ¿la aplicación de Azure está registrada correctamente?"
        );
        console.log(error);
      ...
      msftLoginLogger.info(
        "AuthCode adquirido, procediendo con la autenticación."
      );
      ...
      val.onclick = (e) => {
        ...
        if (Object.keys(ConfigManager.getAuthAccounts()).length === 1) {
          ...
          setOverlayContent(
            ...
            "Para usar el iniciador, debe iniciar sesión en al menos una cuenta. Deberás volver a iniciar sesión después.<br><br>¿Estás seguro de que deseas cerrar la sesión?",
            "Si",
            "Cancelar"
          );
          setOverlayHandler(() => {
...
function processLogOut(val, isLastAccount) {
  ...
  } else if (targetAcc.type === "microsoft") {
  ...
  if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
    switchView(getCurrentView(), VIEWS.settings, 500, 500, () => {
      if (arguments_.length > 1 && arguments_[1] === MSFT_ERROR.NOT_FINISHED) {
        // User cancelled.
        msftLogoutLogger.info("Cierre de sesión cancelado por el usuario.");
        return;
      }
      ...
      setOverlayContent(
        "Algo salió mal",
        "El cierre de sesión de Microsoft falló. Inténtalo de nuevo.",
        "OK"
      );
      setOverlayHandler(() => {
        toggleOverlay(false);
    ...
    msftLogoutLogger.info("Cierre de sesión exitoso. uuid:", uuid);
...
const settingsCurrentCrackedAccounts = document.getElementById(
  "settingsCurrentCrackedAccounts"
);
...
function populateAuthAccounts() {
  ...
  authKeys.forEach((val) => {
    ...
    if (acc.type === "microsoft") {
      ...
      crackedAccountStr += accHtml;
    }
  });

  settingsCurrentMicrosoftAccounts.innerHTML = microsoftAuthAccountStr;
  settingsCurrentMojangAccounts.innerHTML = mojangAuthAccountStr;
  settingsCurrentCrackedAccounts.innerHTML = crackedAccountStr;
}
  ...
  Array.from(sEls).map((v, index, arr) => {
    v.onclick = async () => {
      ...
      if (res) {
        ...
        setOverlayContent(
          `No se pudo eliminar<br>Modificación desplegable ${fullName}`,
          "Asegúrate de que el archivo no esté en uso y vuelve a intentarlo.",
          "Vale"
        );
    ...
    if (dropinUI != null) {
      ...
      if (
        ...
        DropinModUtil.toggleDropinMod(
          ...
          if (!isOverlayVisible()) {
            setOverlayContent(
              "No se pudo activar/desactivar<br>uno o más mods importados por ti",
              err.message,
              "Vale"
            );
            setOverlayHandler(null);
...
function populateJavaReqDesc(server) {
  settingsJavaReqDesc.innerHTML = `Requiere Java ${server.effectiveJavaOptions.suggestedMajor} x64.`;
}
...
function populateReleaseNotes() {
  $.ajax({
    ...
    settingsAboutChangelogText.innerHTML =
      "Error al cargar las notas de la versión.";
  });
}
  ...
  if (data != null) {
    ...
    if (process.platform === "darwin") {
      settingsUpdateButtonStatus(
        'Descargar desde GitHub<span style="font-size: 10px;color: gray;text-shadow: none !important;">Cierra el lanzador y ejecuta el dmg para actualizar.</span>',
        false,
        () => {
    ...
    settingsUpdateTitle.innerHTML = "Estás ejecutando la última versión";
    ...
    settingsUpdateButtonStatus("Buscar actualizaciones", false, () => {
      if (!isDev) {
        ...
        settingsUpdateButtonStatus("Comprobando actualizaciones...", true);
      }
    });
  }
}
