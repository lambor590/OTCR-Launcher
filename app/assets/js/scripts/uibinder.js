...
const VIEWS = {
  ...
  crackedLogin: "#crackedLoginContainer",
};
...
async function showMainUI(data) {
  if (!isDev) {
    loggerAutoUpdater.info("Inicializando...");
    ipcRenderer.send(
      "autoUpdateAction",
      "initAutoUpdater",
  ...
  setTimeout(() => {
    ...
    const isCracked = ConfigManager.getSelectedAccount()?.type === "cracked";

    // If this is enabled in a development environment we'll get ratelimited.
    // The relaunch frequency is usually far too high.
    if (!isDev && isLoggedIn && !isCracked) {
...
async function validateSelectedAccount() {
  ...
  if (selectedAcc != null) {
    ...
    if (selectedAcc.type === "cracked") return;
    ...
    if (!val) {
      ...
      setOverlayContent(
        ...
        `No pudimos actualizar el inicio de sesión de <strong>${
          selectedAcc.displayName
        }</strong>. Por favor ${
          accLen > 0 ? "selecciona otra cuenta o " : ""
