...
class ProcessBuilder {
  ...
  build() {
    ...
    logger.info("Argumentos de inicio:", args);
    ...
    child.on("close", (code, signal) => {
      logger.info("Salió con código", code);
      fs.remove(tempNativePath, (err) => {
        if (err) {
          logger.warn("Error al eliminar el directorio temporal", err);
        } else {
          logger.info("El directorio temporal se eliminó con éxito.");
        }
      });
    });
  ...
  _constructJVMArguments112(mods, tempNativePath) {
    ...
    if (process.platform === "darwin") {
      args.push("-Xdock:name=OTCRLauncher");
      args.push(
  ...
  _constructJVMArguments113(mods, tempNativePath) {
        ...
        if (argDiscovery.test(args[i])) {
          ...
          switch (identifier) {
            ...
            case "natives_directory":
              ...
              break;
            case "launcher_name":
              val = args[i].replace(argDiscovery, "OTCR-Launcher-V3");
              break;
    ...
    try {
      ...
      logger.error(err);
      logger.error(
        "El formato de la versión de Forge cambió... asumiendo que la conexión automática funciona."
      );
      logger.debug("Versión de Forge:", this.forgeData.id);
    }

    if (isAutoconnectBroken) {
      logger.error(
        "La conexión automática del servidor está deshabilitada en Forge 1.15.2 para compilaciones anteriores a la 31.2.15 debido a un problema de desbordamiento de pila de OpenGL."
      );
      logger.error("¡Actualice su versión de Forge al menos a 31.2.15!");
    } else {
          ...
          for (let i = 0; i < zipEntries.length; i++) {
            ...
            if (!shouldExclude) {
              fs.writeFile(
                ...
                (err) => {
                  if (err) {
                    logger.error("Error al extraer la biblioteca nativa:", err);
                  }
                }
              );
            }
